<x-layouts.app :title="'Create Product'">
    <div class="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-zinc-900">
        <div class="container mx-auto px-4 py-6">
            <h1 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200 text-center">Create Product</h1>
            @if ($errors->any())
            ...
            @endif
            <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="max-w-lg mx-auto bg-white dark:bg-zinc-800 p-6 rounded-lg shadow-md">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Name</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" required>
                    @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label for="description" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Description</label>
                    <textarea name="description" id="description" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100">{{ old('description') }}</textarea>
                    @error('description') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label for="price" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Price</label>
                    <input type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price') }}" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" required>
                    @error('price') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4">
                    <label for="stock_quantity" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Stock Quantity</label>
                    <input type="number" name="stock_quantity" id="stock_quantity" min="0" value="{{ old('stock_quantity') }}" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" required>
                ...
                <div class="mb-4">
                    ...
                    <select name="waste_category_id" id="waste_category_id" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" required>
                        <option value="1" {{ old('waste_category_id') == 1 ? 'selected' : '' }}>Plastic</option>
                        <option value="2" {{ old('waste_category_id') == 2 ? 'selected' : '' }}>Paper</option>
                        <option value="3" {{ old('waste_category_id') == 3 ? 'selected' : '' }}>Glass</option>
                        <option value="4" {{ old('waste_category_id') == 4 ? 'selected' : '' }}>Metal</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="recycling_process_id" id="recycling_process_id" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" required>
                        <option value="1" {{ old('recycling_process_id') == 1 ? 'selected' : '' }}>Mechanical Recycling</option>
                        <option value="2" {{ old('recycling_process_id') == 2 ? 'selected' : '' }}>Chemical Recycling</option>
                        <option value="3" {{ old('recycling_process_id') == 3 ? 'selected' : '' }}>Composting</option>
                        <option value="4" {{ old('recycling_process_id') == 4 ? 'selected' : '' }}>Incineration</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="specifications" id="specifications" class="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-700 text-gray-900 dark:text-gray-100" placeholder='{"material": "plastic", "weight": "500g"}'>{{ old('specifications') }}</textarea>
                    @error('specifications') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4 flex items-center">
                    <input type="checkbox" name="is_available" id="is_available" value="1" class="h-4 w-4 text-green-400 focus:ring-green-500 border-gray-300 dark:border-zinc-600 rounded" {{ old('is_available') ? 'checked' : '' }}>
