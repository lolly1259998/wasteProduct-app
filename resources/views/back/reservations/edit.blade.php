        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card shadow-lg">
                    <div class="card-body">
                        <h1 class="text-2xl font-bold mb-6 text-green-700 dark:text-green-300 text-center">Edit Reservation #{{ $reservation->id }}</h1>
                        @if ($errors->any())
                        ...
                        @endif
                        <form action="{{ route('back.reservations.update', $reservation) }}" method="POST" class="space-y-4">
                            @csrf
                            @method('PUT')
                            <!-- User -->
                            <div class="mb-3">
                                ...
                                <select name="product_id" id="product_id" class="form-control bg-white dark:bg-zinc-700 text-gray-900 dark:text-gray-100 border-teal-300 dark:border-teal-600 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400" required>
                                    @if(isset($products) && !empty($products))
                                        @foreach($products as $id => $product)
                                            <option value="{{ $id }}" {{ old('product_id', $reservation->product_id) == $id ? 'selected' : '' }}>{{ $product['name'] }}</option>
                                        @endforeach
                                    @else
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" name="reserved_until" id="reserved_until" value="{{ old('reserved_until', $reservation->reserved_until->format('Y-m-d\TH:i')) }}" class="form-control bg-white dark:bg-zinc-700 text-gray-900 dark:text-gray-100 border-teal-300 dark:border-teal-600 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400" required>
