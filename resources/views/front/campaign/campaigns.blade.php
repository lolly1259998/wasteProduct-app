    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div class="modal-header border-0 bg-success text-white py-3">
                <h5 class="modal-title fw-bold ms-2" id="campaignModalLabel"></h5>
            ...
            <div class="modal-body p-0">
                <div class="row g-0">
                    ...
                    <div class="col-md-6 p-4">
                        <h4 class="fw-bold text-success mb-3" id="campaignModalTitle"></h4>
...
$(document).ready(function () {
        ...
        success: function (data) {
            ...
            data.forEach(campaign => {
                ...
                const statusClass = 
                    ...
                    status === 'draft' ? 'bg-warning text-dark' : 'bg-secondary text-white';
                ...
                @auth
                    $.ajax({
                        url: `/campaigns/${campaign.id}/check`,
                        method: 'GET',
                        async: false, // Synchronous to ensure badge display
                        success: function (res) {
                            if (res.joined) {
                                joinedBadge = '<span class="joined-badge">Joined ✓</span>';
                            }
                        },
                        error: function () {
                            console.log('Error checking participation for campaign ' + campaign.id);
                        }
                    });
                @endauth

                const cardHtml = `
                    <div class="col-md-4 campaign-item" data-status="${status}" data-campaign-id="${campaign.id}">
                        <div class="campaign-card">
                            ${joinedBadge}
                            <img src="${imageSrc}" alt="${campaign.title || 'Campaign Image'}" 
                                onerror="this.outerHTML='<div class=\\'image-error\\'>Image not found</div>'">
                            <span class="campaign-status ${statusClass}">${statusText}</span>
                            <div class="card-body">
                                <h5><i class="bi ${config.icon} me-2" style="color:${config.color}"></i>${campaign.title || 'N/A'}</h5>
                                ...
                                <a href="#" class="btn btn-outline-success btn-sm see-more-btn" 
                                    ...
                                    data-title="${campaign.title || 'N/A'}"
                                    data-description="${campaign.description || 'No description available'}"
                                    data-image="${imageSrc}"
    ...
    // Modal content and participation check
    $('#campaignModal').on('show.bs.modal', function (e) {
        ...
        $('#campaignModalTitle').text(btn.data('title'));
        ...
        @auth
            $.ajax({
                ...
                success: function (res) {
                    if (res.joined) {
                        $('#campaignModalBtn')
                            .removeClass('btn-success').addClass('btn-outline-success disabled')
                            .html('<i class="bi bi-check-circle me-2"></i>Joined ✓');
                    } else {
                        if (status === 'active') {
                            $('#campaignModalBtn')
                                .removeClass('btn-outline-success disabled').addClass('btn-success')
                                .html('<i class="bi bi-person-plus me-2"></i>Participate');
                        } else {
                            $('#campaignModalBtn')
                                .removeClass('btn-success').addClass('btn-outline-success disabled')
                                .html('<i class="bi bi-lock me-2"></i>Cannot Join');
                        }
                    }
                },
                error: function () {
                    console.log('Error checking participation for campaign ' + campaignId);
                    // Default to "Cannot Join" if status check fails
                    $('#campaignModalBtn')
                        .removeClass('btn-success').addClass('btn-outline-success disabled')
                        .html('<i class="bi bi-lock me-2"></i>Cannot Join');
                }
    ...
    $('#campaignModalBtn').on('click', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                const campaignCard = $(`.campaign-item[data-campaign-id="${campaignId}"] .campaign-card`);
                
                if (res.joined) {
                    // Update modal button to "Joined ✓"
                    btn.removeClass('btn-success').addClass('btn-outline-success disabled')
                        .html('<i class="bi bi-check-circle me-2"></i>Joined ✓');
                    
                    // Add "Joined ✓" badge to the campaign card
                    campaignCard.find('.joined-badge').remove(); // Remove existing badge if any
                    campaignCard.prepend('<span class="joined-badge">Joined ✓</span>');
                } else {
                    // Update modal button to "Participate"
                    btn.removeClass('btn-outline-success disabled').addClass('btn-success')
                        .html('<i class="bi bi-person-plus me-2"></i>Participate');
                    
                    // Remove "Joined ✓" badge from the campaign card
                    campaignCard.find('.joined-badge').remove();
                }
