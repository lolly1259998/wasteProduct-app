        ...
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card shadow-lg">
                    <div class="card-body">
                        <h1 class="text-2xl font-bold mb-6 text-green-700 dark:text-green-300 text-center">Modifier le Déchet ♻️</h1>
                        ...
                        <form action="{{ route('wastes.update', $waste->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            @method('PUT')
                            <!-- Type -->
                            <div class="mb-3">
                                <label for="type" class="form-label text-lg font-semibold text-gray-800 dark:text-gray-200">Type</label>
                                <input type="text" name="type" id="type" value="{{ old('type', $waste->type) }}" class="form-control bg-white dark:bg-zinc-700 text-gray-900 dark:text-gray-100 border-teal-300 dark:border-teal-600 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400" required>
                                @error('type')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" name="weight" id="weight" value="{{ old('weight', $waste->weight) }}" class="form-control bg-white dark:bg-zinc-700 text-gray-900 dark:text-gray-100 border-teal-300 dark:border-teal-600 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400" required>
                                @error('weight')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if ($waste->image_path)
                                    <p class="mt-2 text-gray-600 dark:text-gray-400">Image actuelle : <a href="{{ asset('storage/' . $waste->image_path) }}" target="_blank">{{ $waste->image_path }}</a></p>
                                @endif
                                @error('image')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-success btn-md">Mettre à jour</button>
                            </div>
                        </form>
                    </div>
                </div>
